	...
	
{
	...
	/// </summary>
	public partial class Install2Segment
	...
	{	
		...
		public void CompareSVN()
		{
			...
			else
			{
				...
				if (OSbit == 32)
					...
					DownloadBootstrap.StartInfo.Arguments="-t -o StrictHostKeyChecking=no " + LinuxUser + "@" + LinuxIP + " wget http://ausreldev.ultra-ats.com/bootstrap.py";
					DownloadBootstrap.Start();
					Delay.Seconds(1);
					Keyboard.Press(LinuxPass);
					...
					if (buildtype == "nightly")
						...
						UpdateLinux.StartInfo.Arguments="-t -o StrictHostKeyChecking=no " + LinuxUser + "@" + LinuxIP + " sudo python bootstrap.py -y -n 100072";
						UpdateLinux.Start();
						Delay.Seconds(1);
						Keyboard.Press(LinuxPass);
						Keyboard.Press("{Return}");
						...
						UpdateLinux.EnableRaisingEvents=false;
						UpdateLinux.StartInfo.FileName=@"c:/Program Files/OpenSSH/bin/ssh.exe";
						UpdateLinux.StartInfo.Arguments="-t -o StrictHostKeyChecking=no " + LinuxUser + "@" + LinuxIP + " sudo python bootstrap.py -y -n 100082";
						UpdateLinux.Start();
						Delay.Seconds(1);
						Keyboard.Press(LinuxPass);
						Keyboard.Press("{Return}");
				    ...
				    CopyVersion.StartInfo.Arguments=@"-o StrictHostKeyChecking=no " + LinuxUser + "@" + LinuxIP + ":/opt/ats/adsi/current/version BuildInfo/LinuxVersion";
				    CopyVersion.Start();
				    Delay.Seconds(1);
				    Keyboard.Press(LinuxPass);
				    Keyboard.Press("{Return}");
					...
					LinuxInstall.StartInfo.Arguments="-t -o StrictHostKeyChecking=no " + LinuxUser + "@" + LinuxIP + " wget http://ausreldev.ultra-ats.com/bootstrap.py";
					LinuxInstall.Start();
					Delay.Seconds(1);
					Keyboard.Press(LinuxPass);
					Keyboard.Press("{Return}");
        ...
        {
        	Report.Info("Pinging " + LinuxIP + " to ensure it is alive");
        	Ping ping = new Ping();
        	PingReply reply = ping.Send(LinuxIP, 500);
        	if (reply.Status == IPStatus.Success)
        	{
        		Report.Success(LinuxIP + " is alive");
        	}
        	else
        	{
        		Report.Error("Unable to ping " + LinuxIP + ". Check the remote segment and ensure it is powered on.");
        		Thread.ResetAbort();
        	}
        }
