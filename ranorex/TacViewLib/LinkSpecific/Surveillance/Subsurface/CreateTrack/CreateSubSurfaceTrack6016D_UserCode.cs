...
namespace TacViewLib.LinkSpecific.Surveillance.Subsurface.CreateTrack
{
	...
	public partial class CreateSubSurfaceTrack6016D
	{
		...
		public void IDval()
		{
			repo.FormMain__SubSurface_TN___CREATE.ListItemIdentity.Click("CenterLeft");
			...
			Accessible accElement = new Accessible(repo.FormMain__SubSurface_TN___CREATE.ListItemIdentity);  
   			string IDValue = accElement.Value;
   			ValidateTrack.ValidateSubSurfaceTrack6016D.ID = IDValue;
		}
		...
		public void Platval()
		{
			repo.FormMain__SubSurface_TN___CREATE.ListItemPlatform.Click("CenterLeft");
		    Keyboard.Press(Keys.Tab);			
			Keyboard.Press("{LMenu down}{Down}{LMenu up}");
			bool found = false;
			ValidateTrack.ValidateSubSurfaceTrack6016D.PlatFlag=false;
        	...
        	if (!found)
        	{
	        	ValidateTrack.ValidateSubSurfaceTrack6016D.PlatFlag=true;
		...
		public void Actval()
		{
			repo.FormMain__SubSurface_TN___CREATE.ListItemActivity.Click("CenterLeft");
		    Keyboard.Press(Keys.Tab);			
			Keyboard.Press("{LMenu down}{Down}{LMenu up}");
			bool found = false;
			ValidateTrack.ValidateSubSurfaceTrack6016D.ActFlag=false;
        	foreach( ListItem item in repo.ListRealView.Self.Items )
        	{
           		if ( item.Text == Act )
           		{
        	...
        	if (!found)
        	{
	        	ValidateTrack.ValidateSubSurfaceTrack6016D.ActFlag=true;
	           	repo.ListRealView.Self.Items[1].Select();
	       		repo.ListRealView.Self.Items[1].Click();
        	}  
		}
		...
		{
			repo.FormMain__SubSurface_TN___CREATE.ListItemSpecific_Type.Click("CenterLeft");
		    Keyboard.Press(Keys.Tab);			
			Keyboard.Press("{LMenu down}{Down}{LMenu up}");
			bool found = false;
			ValidateTrack.ValidateSubSurfaceTrack6016D.StypeFlag=false;
        	foreach( ListItem item in repo.ListRealView.Self.Items )
        	{
           		if ( item.Text == Stype )
           		{
        	...
        	if (!found)
        	{
	        	 ValidateTrack.ValidateSubSurfaceTrack6016D.StypeFlag=true;
	           	 repo.ListRealView.Self.Items[1].Select();
	       		 repo.ListRealView.Self.Items[1].Click();
        	}          	
		}
		...
		static CreateSubSurfaceTrack6016D()
		{
		...
		{			
			...
			string track1 = accElement.Name;
	 		...
	 		ValidateTrack.ValidateSubSurfaceTrack6016D.TrackNum = track1;
