...
def send_data(comPort, baudrate, bitsPerByte, busLoad, processLoops, returnBuffer, messageQueue):
    dataLogArray = []
    byteStreamCounter = 0
    sendByteCounter = 0
    ...
    amountOfBytesInStream = calc_bytes_to_send(baudrate, bitsPerByte, busLoad)
    busLoadWaitTime = bus_load_wait_time(baudrate, amountOfBytesInStream, bitsPerByte)

    csvfile = open('UartResultsTxInProcess.csv', 'wt')

    while processLoopCounter < processLoops:
        #com_port1.write(chr(sendByteCounter)) # Use this for python2.7
        com_port1.write(chr(sendByteCounter).encode())  # Use this for python3 ->
        array = []
        array.append('Tx:')
        array.append(logCounter)
        array.append(hex(sendByteCounter))
        ...
        byteStreamCounter += 1
        sendByteCounter += 1
        if sendByteCounter == 128:
            sendByteCounter = 0
        # inform the RX process that the data block has been sent and is ready for logging
        # also save the data in the log file
        if byteStreamCounter == amountOfBytesInStream + 1:
            sleepStartTime = time.time()
            messageQueue.put(sendByteCounter-1)
            print('Put onto Queue: ' + repr(sendByteCounter-1))
            ...
            print('Finished %d / %d transmit loops' % (processLoopCounter + 1, processLoops))
            dataLogArray = []
            byteStreamCounter = 0
